const App = {
    setup() {
        ...
        const validateForm = function () {
            state.errors.name = ''; state.errors.jobTitle = ''; state.errors.phoneNumber = ''; state.errors.emailAddress = ''; state.errors.customerId = '';
            let isValid = true;
        ...
        const resetFormState = () => {
            ...
            state.emailAddress = ''; state.description = ''; state.customerId = null;
            state.errors = { name: '', jobTitle: '', phoneNumber: '', emailAddress: '', customerId: '' };
        };

        const services = {
            getMainData: async () => { try { return await AxiosManager.get('/CustomerContact/GetCustomerContactList', {}); } catch (e) { throw e; } },
            createMainData: async (name, jobTitle, phoneNumber, emailAddress, description, customerId, createdById) => { try { return await AxiosManager.post('/CustomerContact/CreateCustomerContact', { name, jobTitle, phoneNumber, emailAddress, description, customerId, createdById }); } catch (e) { throw e; } },
            updateMainData: async (id, name, jobTitle, phoneNumber, emailAddress, description, customerId, updatedById) => { try { return await AxiosManager.post('/CustomerContact/UpdateCustomerContact', { id, name, jobTitle, phoneNumber, emailAddress, description, customerId, updatedById }); } catch (e) { throw e; } },
            deleteMainData: async (id, deletedById) => { try { return await AxiosManager.post('/CustomerContact/DeleteCustomerContact', { id, deletedById }); } catch (e) { throw e; } },
            getCustomerListLookupData: async () => { try { return await AxiosManager.get('/Customer/GetCustomerList', {}); } catch (e) { throw e; } },
        };
        ...
        const customerListLookup = {
            ...
            create: () => {
                if (state.customerListLookupData && Array.isArray(state.customerListLookupData)) {
                    customerListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.customerListLookupData, fields: { value: 'id', text: 'name' }, placeholder: 'Select a Customer', change: (e) => { state.customerId = e.value; } });
        ...
        Vue.watch(() => state.customerId, () => { state.errors.customerId = ''; customerListLookup.refresh(); });

        const handler = {
            openAdd: () => { state.deleteMode = false; state.mainTitle = 'Add Customer Contact'; resetFormState(); showModal(); },
            openEdit: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = false; state.mainTitle = 'Edit Customer Contact'; mainGrid._populateState(row); showModal(); },
            openDelete: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = true; state.mainTitle = 'Delete Customer Contact?'; mainGrid._populateState(row); showModal(); },
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['CustomerContacts']); await SecurityManager.validateToken();
        ...
        const mainGrid = {
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Customer Contacts', exportOptions: { columns: ':visible' } }],
        ...
        return { mainModalRef, nameRef, numberRef, jobTitleRef, phoneNumberRef, emailAddressRef, customerIdRef, state, handler };
    }
};
