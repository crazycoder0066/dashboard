const App = {
    setup() {
        ...
        const showItemModal = () => { bootstrap.Modal.getOrCreateInstance(document.getElementById('NegativeAdjustmentItemModal'), { backdrop: 'static', keyboard: false }).show(); };
        const hideItemModal = () => { bootstrap.Modal.getInstance(document.getElementById('NegativeAdjustmentItemModal'))?.hide(); };
        ...
        const negativeAdjustmentStatusListLookup = { obj: null, create: () => { if (state.negativeAdjustmentStatusListLookupData && Array.isArray(state.negativeAdjustmentStatusListLookupData)) { negativeAdjustmentStatusListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.negativeAdjustmentStatusListLookupData, fields: { value: 'id', text: 'name' }, placeholder: 'Select Status', change: (e) => { state.status = e.value; } }); negativeAdjustmentStatusListLookup.obj.appendTo(statusRef.value); } }, refresh: () => { if (negativeAdjustmentStatusListLookup.obj) negativeAdjustmentStatusListLookup.obj.value = state.status; } };
        Vue.watch(() => state.status, () => { negativeAdjustmentStatusListLookup.refresh(); state.errors.status = ''; });
        ...
        const services = {
            getMainData: async () => AxiosManager.get('/NegativeAdjustment/GetNegativeAdjustmentList', {}),
            createMainData: async (adjustmentDate, description, status, createdById) => AxiosManager.post('/NegativeAdjustment/CreateNegativeAdjustment', { adjustmentDate, description, status, createdById }),
            updateMainData: async (id, adjustmentDate, description, status, updatedById) => AxiosManager.post('/NegativeAdjustment/UpdateNegativeAdjustment', { id, adjustmentDate, description, status, updatedById }),
            deleteMainData: async (id, deletedById) => AxiosManager.post('/NegativeAdjustment/DeleteNegativeAdjustment', { id, deletedById }),
            getNegativeAdjustmentStatusListLookupData: async () => AxiosManager.get('/NegativeAdjustment/GetNegativeAdjustmentStatusList', {}),
            getSecondaryData: async (moduleId) => AxiosManager.get('/InventoryTransaction/NegativeAdjustmentGetInvenTransList?moduleId=' + moduleId, {}),
            createSecondaryData: async (moduleId, warehouseId, productId, movement, createdById) => AxiosManager.post('/InventoryTransaction/NegativeAdjustmentCreateInvenTrans', { moduleId, warehouseId, productId, movement, createdById }),
            updateSecondaryData: async (id, warehouseId, productId, movement, updatedById) => AxiosManager.post('/InventoryTransaction/NegativeAdjustmentUpdateInvenTrans', { id, warehouseId, productId, movement, updatedById }),
            deleteSecondaryData: async (id, deletedById) => AxiosManager.post('/InventoryTransaction/NegativeAdjustmentDeleteInvenTrans', { id, deletedById }),
        ...
        const methods = {
            ...
            populateNegativeAdjustmentStatusListLookupData: async () => { const r = await services.getNegativeAdjustmentStatusListLookupData(); state.negativeAdjustmentStatusListLookupData = r?.data?.content?.data; },
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) { state.mainTitle = 'Edit Negative Adjustment'; state.id = response?.data?.content?.data.id ?? ''; state.number = response?.data?.content?.data.number ?? ''; await methods.populateSecondaryData(state.id); secondaryGrid.refresh(); state.showComplexDiv = true; Swal.fire({ icon: 'success', title: 'Save Successful', timer: 2000, showConfirmButton: false }); }
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['NegativeAdjustments']); await SecurityManager.validateToken();
                ...
                await methods.populateNegativeAdjustmentStatusListLookupData(); negativeAdjustmentStatusListLookup.create();
        ...
        const mainGrid = {
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Negative Adjustments', exportOptions: { columns: ':visible' } }],
