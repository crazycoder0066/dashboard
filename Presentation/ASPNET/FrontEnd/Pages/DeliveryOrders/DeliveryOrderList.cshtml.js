const App = {
    setup() {
        const state = Vue.reactive({
            ...
            salesOrderId: null,
            status: null,
            errors: { deliveryDate: '', salesOrderId: '', status: '', description: '', itemWarehouseId: '', itemProductId: '', itemMovement: '' },
        ...
        const showItemModal = () => { bootstrap.Modal.getOrCreateInstance(document.getElementById('DeliveryOrderItemModal'), { backdrop: 'static', keyboard: false }).show(); };
        const hideItemModal = () => { bootstrap.Modal.getInstance(document.getElementById('DeliveryOrderItemModal'))?.hide(); };

        const validateForm = function () {
            state.errors.deliveryDate = ''; state.errors.salesOrderId = ''; state.errors.status = '';
            let isValid = true;
            if (!state.deliveryDate) { state.errors.deliveryDate = 'Delivery date is required.'; isValid = false; }
            if (!state.salesOrderId) { state.errors.salesOrderId = 'Sales Order is required.'; isValid = false; }
            if (!state.status) { state.errors.status = 'Status is required.'; isValid = false; }
            return isValid;
        };

        const resetFormState = () => {
            state.id = ''; state.number = ''; state.deliveryDate = ''; state.description = ''; state.salesOrderId = null; state.status = null;
            state.errors = { deliveryDate: '', salesOrderId: '', status: '', description: '', itemWarehouseId: '', itemProductId: '', itemMovement: '' }; state.secondaryData = [];
        };
        ...
        const orderDatePicker = { obj: null, create: () => { orderDatePicker.obj = new ej.calendars.DatePicker({ placeholder: 'Select Date', format: 'yyyy-MM-dd', value: state.deliveryDate ? new Date(state.deliveryDate) : null, change: (e) => { state.deliveryDate = e.value; } }); orderDatePicker.obj.appendTo(deliveryDateRef.value); }, refresh: () => { if (orderDatePicker.obj) orderDatePicker.obj.value = state.deliveryDate ? new Date(state.deliveryDate) : null; } };
        Vue.watch(() => state.deliveryDate, () => { orderDatePicker.refresh(); state.errors.deliveryDate = ''; });
        ...
        const salesOrderListLookup = { obj: null, create: () => { if (state.salesOrderListLookupData && Array.isArray(state.salesOrderListLookupData)) { salesOrderListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.salesOrderListLookupData, fields: { value: 'id', text: 'number' }, placeholder: 'Select Sales Order', filterBarPlaceholder: 'Search', sortOrder: 'Ascending', allowFiltering: true, filtering: (e) => { e.preventDefaultAction = true; let q = new ej.data.Query(); if (e.text !== '') q = q.where('number', 'startsWith', e.text, true); e.updateData(state.salesOrderListLookupData, q); }, change: (e) => { state.salesOrderId = e.value; } }); salesOrderListLookup.obj.appendTo(salesOrderIdRef.value); } }, refresh: () => { if (salesOrderListLookup.obj) salesOrderListLookup.obj.value = state.salesOrderId; } };
        Vue.watch(() => state.salesOrderId, () => { salesOrderListLookup.refresh(); state.errors.salesOrderId = ''; });

        const statusListLookup = { obj: null, create: () => { if (state.statusListLookupData && Array.isArray(state.statusListLookupData)) { statusListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.statusListLookupData, fields: { value: 'id', text: 'name' }, placeholder: 'Select Status', change: (e) => { state.status = e.value; } }); statusListLookup.obj.appendTo(statusRef.value); } }, refresh: () => { if (statusListLookup.obj) statusListLookup.obj.value = state.status; } };
        ...
        const services = {
            getMainData: async () => AxiosManager.get('/DeliveryOrder/GetDeliveryOrderList', {}),
            createMainData: async (deliveryDate, description, status, salesOrderId, createdById) => AxiosManager.post('/DeliveryOrder/CreateDeliveryOrder', { deliveryDate, description, status, salesOrderId, createdById }),
            updateMainData: async (id, deliveryDate, description, status, salesOrderId, updatedById) => AxiosManager.post('/DeliveryOrder/UpdateDeliveryOrder', { id, deliveryDate, description, status, salesOrderId, updatedById }),
            deleteMainData: async (id, deletedById) => AxiosManager.post('/DeliveryOrder/DeleteDeliveryOrder', { id, deletedById }),
            getSalesOrderListLookupData: async () => AxiosManager.get('/SalesOrder/GetSalesOrderList', {}),
            getDeliveryOrderStatusListLookupData: async () => AxiosManager.get('/DeliveryOrder/GetDeliveryOrderStatusList', {}),
            getSecondaryData: async (moduleId) => AxiosManager.get('/InventoryTransaction/DeliveryOrderGetInvenTransList?moduleId=' + moduleId, {}),
            createSecondaryData: async (moduleId, warehouseId, productId, movement, createdById) => AxiosManager.post('/InventoryTransaction/DeliveryOrderCreateInvenTrans', { moduleId, warehouseId, productId, movement, createdById }),
            updateSecondaryData: async (id, warehouseId, productId, movement, updatedById) => AxiosManager.post('/InventoryTransaction/DeliveryOrderUpdateInvenTrans', { id, warehouseId, productId, movement, updatedById }),
            deleteSecondaryData: async (id, deletedById) => AxiosManager.post('/InventoryTransaction/DeliveryOrderDeleteInvenTrans', { id, deletedById }),
        ...
        const methods = {
            populateMainData: async () => { const r = await services.getMainData(); state.mainData = r?.data?.content?.data.map(item => ({ ...item, deliveryDate: new Date(item.deliveryDate), createdAtUtc: new Date(item.createdAtUtc) })); },
            populateSalesOrderListLookupData: async () => { const r = await services.getSalesOrderListLookupData(); state.salesOrderListLookupData = r?.data?.content?.data; },
            populateDeliveryOrderStatusListLookupData: async () => { const r = await services.getDeliveryOrderStatusListLookupData(); state.statusListLookupData = r?.data?.content?.data; },
            ...
            onMainModalHidden: () => { state.errors.deliveryDate = ''; state.errors.salesOrderId = ''; state.errors.status = ''; },
        };

        const mainModal = { obj: null, create: () => { mainModal.obj = new bootstrap.Modal(mainModalRef.value, { backdrop: 'static', keyboard: false }); } };

        const handler = {
            openAdd: () => { state.deleteMode = false; state.mainTitle = 'Add Delivery Order'; resetFormState(); state.showComplexDiv = false; showModal(); },
            openEdit: async () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = false; state.mainTitle = 'Edit Delivery Order'; state.id = row.id ?? ''; state.number = row.number ?? ''; state.deliveryDate = row.deliveryDate ? new Date(row.deliveryDate) : null; state.description = row.description ?? ''; state.salesOrderId = row.salesOrderId ?? ''; state.status = String(row.status ?? ''); await methods.populateSecondaryData(row.id); secondaryGrid.refresh(); state.showComplexDiv = true; showModal(); },
            openDelete: async () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = true; state.mainTitle = 'Delete Delivery Order?'; state.id = row.id ?? ''; state.number = row.number ?? ''; state.deliveryDate = row.deliveryDate ? new Date(row.deliveryDate) : null; state.description = row.description ?? ''; state.salesOrderId = row.salesOrderId ?? ''; state.status = String(row.status ?? ''); await methods.populateSecondaryData(row.id); secondaryGrid.refresh(); state.showComplexDiv = false; showModal(); },
            printPdf: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; window.open('/DeliveryOrders/DeliveryOrderPdf?id=' + (row.id ?? ''), '_blank'); },
            exportExcel: () => { mainGrid.obj?.button(0).trigger(); },
            handleSubmit: async function () {
                try {
                    state.isSubmitting = true; await new Promise(r => setTimeout(r, 300));
                    if (!validateForm()) return;
                    const response = state.id === '' ? await services.createMainData(state.deliveryDate, state.description, state.status, state.salesOrderId, StorageManager.getUserId()) : state.deleteMode ? await services.deleteMainData(state.id, StorageManager.getUserId()) : await services.updateMainData(state.id, state.deliveryDate, state.description, state.status, state.salesOrderId, StorageManager.getUserId());
                    if (response.data.code === 200) {
                        await methods.populateMainData(); mainGrid.refresh();
                        if (!state.deleteMode) { state.mainTitle = 'Edit Delivery Order'; state.id = response?.data?.content?.data.id ?? ''; state.number = response?.data?.content?.data.number ?? ''; await methods.populateSecondaryData(state.id); secondaryGrid.refresh(); state.showComplexDiv = true; Swal.fire({ icon: 'success', title: 'Save Successful', timer: 2000, showConfirmButton: false }); }
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['DeliveryOrders']); await SecurityManager.validateToken();
                ...
                await methods.populateSalesOrderListLookupData(); salesOrderListLookup.create();
                await methods.populateDeliveryOrderStatusListLookupData(); statusListLookup.create();
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    columns: [
                        ...
                        { data: 'deliveryDate', title: 'Delivery Date', render: d => d ? new Date(d).toLocaleDateString('en-GB') : '' },
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Delivery Orders', exportOptions: { columns: ':visible' } }],
        ...
        return { mainModalRef, itemModalRef, deliveryDateRef, salesOrderIdRef, statusRef, numberRef, itemWarehouseIdRef, itemProductIdRef, state, handler };
    }
};
