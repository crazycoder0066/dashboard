const App = {
    setup() {
        const state = Vue.reactive({
            ...
            status: null,
            errors: { returnDate: '', deliveryOrderId: '', status: '', description: '', itemWarehouseId: '', itemProductId: '', itemMovement: '' },
        ...
        const showItemModal = () => { bootstrap.Modal.getOrCreateInstance(document.getElementById('SalesReturnItemModal'), { backdrop: 'static', keyboard: false }).show(); };
        const hideItemModal = () => { bootstrap.Modal.getInstance(document.getElementById('SalesReturnItemModal'))?.hide(); };

        const validateForm = () => { state.errors.returnDate = ''; state.errors.deliveryOrderId = ''; state.errors.status = ''; let isValid = true; if (!state.returnDate) { state.errors.returnDate = 'Return date is required.'; isValid = false; } if (!state.deliveryOrderId) { state.errors.deliveryOrderId = 'Delivery Order is required.'; isValid = false; } if (!state.status) { state.errors.status = 'Status is required.'; isValid = false; } return isValid; };
        const resetFormState = () => { state.id = ''; state.number = ''; state.returnDate = ''; state.description = ''; state.deliveryOrderId = null; state.status = null; state.errors = { returnDate: '', deliveryOrderId: '', status: '', description: '', itemWarehouseId: '', itemProductId: '', itemMovement: '' }; state.secondaryData = []; };
        ...
        const deliveryOrderListLookup = { obj: null, create: () => { if (state.deliveryOrderListLookupData && Array.isArray(state.deliveryOrderListLookupData)) { deliveryOrderListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.deliveryOrderListLookupData, fields: { value: 'id', text: 'number' }, placeholder: 'Select Delivery Order', filterBarPlaceholder: 'Search', sortOrder: 'Ascending', allowFiltering: true, filtering: (e) => { e.preventDefaultAction = true; let q = new ej.data.Query(); if (e.text !== '') q = q.where('number', 'startsWith', e.text, true); e.updateData(state.deliveryOrderListLookupData, q); }, change: (e) => { state.deliveryOrderId = e.value; } }); deliveryOrderListLookup.obj.appendTo(deliveryOrderIdRef.value); } }, refresh: () => { if (deliveryOrderListLookup.obj) deliveryOrderListLookup.obj.value = state.deliveryOrderId; } };
        Vue.watch(() => state.deliveryOrderId, () => { deliveryOrderListLookup.refresh(); state.errors.deliveryOrderId = ''; });

        const salesReturnStatusListLookup = { obj: null, create: () => { if (state.salesReturnStatusListLookupData && Array.isArray(state.salesReturnStatusListLookupData)) { salesReturnStatusListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.salesReturnStatusListLookupData, fields: { value: 'id', text: 'name' }, placeholder: 'Select Status', change: (e) => { state.status = e.value; } }); salesReturnStatusListLookup.obj.appendTo(statusRef.value); } }, refresh: () => { if (salesReturnStatusListLookup.obj) salesReturnStatusListLookup.obj.value = state.status; } };
        Vue.watch(() => state.status, () => { salesReturnStatusListLookup.refresh(); state.errors.status = ''; });
        ...
        const services = {
            getMainData: async () => AxiosManager.get('/SalesReturn/GetSalesReturnList', {}),
            createMainData: async (returnDate, description, status, deliveryOrderId, createdById) => AxiosManager.post('/SalesReturn/CreateSalesReturn', { returnDate, description, status, deliveryOrderId, createdById }),
            updateMainData: async (id, returnDate, description, status, deliveryOrderId, updatedById) => AxiosManager.post('/SalesReturn/UpdateSalesReturn', { id, returnDate, description, status, deliveryOrderId, updatedById }),
            deleteMainData: async (id, deletedById) => AxiosManager.post('/SalesReturn/DeleteSalesReturn', { id, deletedById }),
            getDeliveryOrderListLookupData: async () => AxiosManager.get('/DeliveryOrder/GetDeliveryOrderList', {}),
            getSalesReturnStatusListLookupData: async () => AxiosManager.get('/SalesReturn/GetSalesReturnStatusList', {}),
            getSecondaryData: async (moduleId) => AxiosManager.get('/InventoryTransaction/SalesReturnGetInvenTransList?moduleId=' + moduleId, {}),
            createSecondaryData: async (moduleId, warehouseId, productId, movement, createdById) => AxiosManager.post('/InventoryTransaction/SalesReturnCreateInvenTrans', { moduleId, warehouseId, productId, movement, createdById }),
            updateSecondaryData: async (id, warehouseId, productId, movement, updatedById) => AxiosManager.post('/InventoryTransaction/SalesReturnUpdateInvenTrans', { id, warehouseId, productId, movement, updatedById }),
            deleteSecondaryData: async (id, deletedById) => AxiosManager.post('/InventoryTransaction/SalesReturnDeleteInvenTrans', { id, deletedById }),
        ...
        const methods = {
            ...
            populateDeliveryOrderListLookupData: async () => { const r = await services.getDeliveryOrderListLookupData(); state.deliveryOrderListLookupData = r?.data?.content?.data; },
            populateSalesReturnStatusListLookupData: async () => { const r = await services.getSalesReturnStatusListLookupData(); state.salesReturnStatusListLookupData = r?.data?.content?.data; },
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['SalesReturns']); await SecurityManager.validateToken();
                ...
                await methods.populateDeliveryOrderListLookupData(); deliveryOrderListLookup.create();
                await methods.populateSalesReturnStatusListLookupData(); salesReturnStatusListLookup.create();
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    columns: [
                        ...
                        { data: 'deliveryOrderNumber', title: 'Delivery Order' },
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Sales Returns', exportOptions: { columns: ':visible' } }],
        ...
        return { mainModalRef, itemModalRef, returnDateRef, deliveryOrderIdRef, statusRef, numberRef, itemWarehouseIdRef, itemProductIdRef, state, handler };
    }
};
