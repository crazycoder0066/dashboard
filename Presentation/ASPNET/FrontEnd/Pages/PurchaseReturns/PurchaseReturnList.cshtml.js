const App = {
    setup() {
        const state = Vue.reactive({
            ...
            goodsReceiveId: null,
            status: null,
            errors: { returnDate: '', goodsReceiveId: '', status: '', description: '', itemWarehouseId: '', itemProductId: '', itemMovement: '' },
        ...
        const showItemModal = () => { bootstrap.Modal.getOrCreateInstance(document.getElementById('PurchaseReturnItemModal'), { backdrop: 'static', keyboard: false }).show(); };
        const hideItemModal = () => { bootstrap.Modal.getInstance(document.getElementById('PurchaseReturnItemModal'))?.hide(); };

        const validateForm = () => { state.errors.returnDate = ''; state.errors.goodsReceiveId = ''; state.errors.status = ''; let isValid = true; if (!state.returnDate) { state.errors.returnDate = 'Return date is required.'; isValid = false; } if (!state.goodsReceiveId) { state.errors.goodsReceiveId = 'Goods Receive is required.'; isValid = false; } if (!state.status) { state.errors.status = 'Status is required.'; isValid = false; } return isValid; };
        const resetFormState = () => { state.id = ''; state.number = ''; state.returnDate = ''; state.description = ''; state.goodsReceiveId = null; state.status = null; state.errors = { returnDate: '', goodsReceiveId: '', status: '', description: '', itemWarehouseId: '', itemProductId: '', itemMovement: '' }; state.secondaryData = []; };
        ...
        const returnDatePicker = { obj: null, create: () => { returnDatePicker.obj = new ej.calendars.DatePicker({ placeholder: 'Select Date', format: 'yyyy-MM-dd', value: state.returnDate ? new Date(state.returnDate) : null, change: (e) => { state.returnDate = e.value; } }); returnDatePicker.obj.appendTo(returnDateRef.value); }, refresh: () => { if (returnDatePicker.obj) returnDatePicker.obj.value = state.returnDate ? new Date(state.returnDate) : null; } };
        Vue.watch(() => state.returnDate, () => { returnDatePicker.refresh(); state.errors.returnDate = ''; });
        ...
        const goodsReceiveListLookup = { obj: null, create: () => { if (state.goodsReceiveListLookupData && Array.isArray(state.goodsReceiveListLookupData)) { goodsReceiveListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.goodsReceiveListLookupData, fields: { value: 'id', text: 'number' }, placeholder: 'Select Goods Receive', filterBarPlaceholder: 'Search', sortOrder: 'Ascending', allowFiltering: true, filtering: (e) => { e.preventDefaultAction = true; let q = new ej.data.Query(); if (e.text !== '') q = q.where('number', 'startsWith', e.text, true); e.updateData(state.goodsReceiveListLookupData, q); }, change: (e) => { state.goodsReceiveId = e.value; } }); goodsReceiveListLookup.obj.appendTo(goodsReceiveIdRef.value); } }, refresh: () => { if (goodsReceiveListLookup.obj) goodsReceiveListLookup.obj.value = state.goodsReceiveId; } };
        Vue.watch(() => state.goodsReceiveId, () => { goodsReceiveListLookup.refresh(); state.errors.goodsReceiveId = ''; });

        const purchaseReturnStatusListLookup = { obj: null, create: () => { if (state.purchaseReturnStatusListLookupData && Array.isArray(state.purchaseReturnStatusListLookupData)) { purchaseReturnStatusListLookup.obj = new ej.dropdowns.DropDownList({ dataSource: state.purchaseReturnStatusListLookupData, fields: { value: 'id', text: 'name' }, placeholder: 'Select Status', change: (e) => { state.status = e.value; } }); purchaseReturnStatusListLookup.obj.appendTo(statusRef.value); } }, refresh: () => { if (purchaseReturnStatusListLookup.obj) purchaseReturnStatusListLookup.obj.value = state.status; } };
        Vue.watch(() => state.status, () => { purchaseReturnStatusListLookup.refresh(); state.errors.status = ''; });
        ...
        const services = {
            getMainData: async () => AxiosManager.get('/PurchaseReturn/GetPurchaseReturnList', {}),
            createMainData: async (returnDate, description, status, goodsReceiveId, createdById) => AxiosManager.post('/PurchaseReturn/CreatePurchaseReturn', { returnDate, description, status, goodsReceiveId, createdById }),
            updateMainData: async (id, returnDate, description, status, goodsReceiveId, updatedById) => AxiosManager.post('/PurchaseReturn/UpdatePurchaseReturn', { id, returnDate, description, status, goodsReceiveId, updatedById }),
            deleteMainData: async (id, deletedById) => AxiosManager.post('/PurchaseReturn/DeletePurchaseReturn', { id, deletedById }),
            getGoodsReceiveListLookupData: async () => AxiosManager.get('/GoodsReceive/GetGoodsReceiveList', {}),
            getPurchaseReturnStatusListLookupData: async () => AxiosManager.get('/PurchaseReturn/GetPurchaseReturnStatusList', {}),
            getSecondaryData: async (moduleId) => AxiosManager.get('/InventoryTransaction/PurchaseReturnGetInvenTransList?moduleId=' + moduleId, {}),
            createSecondaryData: async (moduleId, warehouseId, productId, movement, createdById) => AxiosManager.post('/InventoryTransaction/PurchaseReturnCreateInvenTrans', { moduleId, warehouseId, productId, movement, createdById }),
            updateSecondaryData: async (id, warehouseId, productId, movement, updatedById) => AxiosManager.post('/InventoryTransaction/PurchaseReturnUpdateInvenTrans', { id, warehouseId, productId, movement, updatedById }),
            deleteSecondaryData: async (id, deletedById) => AxiosManager.post('/InventoryTransaction/PurchaseReturnDeleteInvenTrans', { id, deletedById }),
        ...
        const methods = {
            populateMainData: async () => { const r = await services.getMainData(); state.mainData = r?.data?.content?.data.map(item => ({ ...item, returnDate: new Date(item.returnDate), createdAtUtc: new Date(item.createdAtUtc) })); },
            populateGoodsReceiveListLookupData: async () => { const r = await services.getGoodsReceiveListLookupData(); state.goodsReceiveListLookupData = r?.data?.content?.data; },
            populatePurchaseReturnStatusListLookupData: async () => { const r = await services.getPurchaseReturnStatusListLookupData(); state.purchaseReturnStatusListLookupData = r?.data?.content?.data; },
        ...
        const handler = {
            openAdd: () => { state.deleteMode = false; state.mainTitle = 'Add Purchase Return'; resetFormState(); state.showComplexDiv = false; showModal(); },
            openEdit: async () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = false; state.mainTitle = 'Edit Purchase Return'; state.id = row.id ?? ''; state.number = row.number ?? ''; state.returnDate = row.returnDate ? new Date(row.returnDate) : null; state.description = row.description ?? ''; state.goodsReceiveId = row.goodsReceiveId ?? ''; state.status = String(row.status ?? ''); await methods.populateSecondaryData(row.id); secondaryGrid.refresh(); state.showComplexDiv = true; showModal(); },
            openDelete: async () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = true; state.mainTitle = 'Delete Purchase Return?'; state.id = row.id ?? ''; state.number = row.number ?? ''; state.returnDate = row.returnDate ? new Date(row.returnDate) : null; state.description = row.description ?? ''; state.goodsReceiveId = row.goodsReceiveId ?? ''; state.status = String(row.status ?? ''); await methods.populateSecondaryData(row.id); secondaryGrid.refresh(); state.showComplexDiv = false; showModal(); },
            printPdf: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; window.open('/PurchaseReturns/PurchaseReturnPdf?id=' + (row.id ?? ''), '_blank'); },
            exportExcel: () => { mainGrid.obj?.button(0).trigger(); },
            handleSubmit: async function () {
                try {
                    state.isSubmitting = true; await new Promise(r => setTimeout(r, 300));
                    if (!validateForm()) return;
                    const response = state.id === '' ? await services.createMainData(state.returnDate, state.description, state.status, state.goodsReceiveId, StorageManager.getUserId()) : state.deleteMode ? await services.deleteMainData(state.id, StorageManager.getUserId()) : await services.updateMainData(state.id, state.returnDate, state.description, state.status, state.goodsReceiveId, StorageManager.getUserId());
                    if (response.data.code === 200) {
                        await methods.populateMainData(); mainGrid.refresh();
                        if (!state.deleteMode) { state.mainTitle = 'Edit Purchase Return'; state.id = response?.data?.content?.data.id ?? ''; state.number = response?.data?.content?.data.number ?? ''; await methods.populateSecondaryData(state.id); secondaryGrid.refresh(); state.showComplexDiv = true; Swal.fire({ icon: 'success', title: 'Save Successful', timer: 2000, showConfirmButton: false }); }
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['PurchaseReturns']); await SecurityManager.validateToken();
                ...
                await methods.populateGoodsReceiveListLookupData(); goodsReceiveListLookup.create();
                await methods.populatePurchaseReturnStatusListLookupData(); purchaseReturnStatusListLookup.create();
                returnDatePicker.create(); numberText.create();
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    columns: [
                        ...
                        { data: 'returnDate', title: 'Return Date', render: d => d ? new Date(d).toLocaleDateString('en-GB') : '' },
                        { data: 'goodsReceiveNumber', title: 'Goods Receive' },
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Purchase Returns', exportOptions: { columns: ':visible' } }],
        ...
        return { mainModalRef, itemModalRef, returnDateRef, goodsReceiveIdRef, statusRef, numberRef, itemWarehouseIdRef, itemProductIdRef, state, handler };
    }
};
