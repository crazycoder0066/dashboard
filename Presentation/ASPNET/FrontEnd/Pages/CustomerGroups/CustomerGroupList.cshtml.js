const App = {
    setup() {
        ...
        const services = {
            getMainData: async () => { try { return await AxiosManager.get('/CustomerGroup/GetCustomerGroupList', {}); } catch (e) { throw e; } },
            createMainData: async (name, description, createdById) => { try { return await AxiosManager.post('/CustomerGroup/CreateCustomerGroup', { name, description, createdById }); } catch (e) { throw e; } },
            updateMainData: async (id, name, description, updatedById) => { try { return await AxiosManager.post('/CustomerGroup/UpdateCustomerGroup', { id, name, description, updatedById }); } catch (e) { throw e; } },
            deleteMainData: async (id, deletedById) => { try { return await AxiosManager.post('/CustomerGroup/DeleteCustomerGroup', { id, deletedById }); } catch (e) { throw e; } },
        };
        ...
        const handler = {
            openAdd: () => { state.deleteMode = false; state.mainTitle = 'Add Customer Group'; resetFormState(); showModal(); },
            openEdit: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = false; state.mainTitle = 'Edit Customer Group'; mainGrid._populateState(row); showModal(); },
            openDelete: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = true; state.mainTitle = 'Delete Customer Group?'; mainGrid._populateState(row); showModal(); },
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) { state.mainTitle = 'Edit Customer Group'; state.id = response?.data?.content?.data.id ?? ''; state.name = response?.data?.content?.data.name ?? ''; state.description = response?.data?.content?.data.description ?? ''; }
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['CustomerGroups']); await SecurityManager.validateToken();
        ...
        const mainGrid = {
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Customer Groups', exportOptions: { columns: ':visible' } }],
