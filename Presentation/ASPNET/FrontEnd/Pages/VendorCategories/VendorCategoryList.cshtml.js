const App = {
    setup() {
        ...
        const services = {
            getMainData: async () => { try { return await AxiosManager.get('/VendorCategory/GetVendorCategoryList', {}); } catch (e) { throw e; } },
            createMainData: async (name, description, createdById) => { try { return await AxiosManager.post('/VendorCategory/CreateVendorCategory', { name, description, createdById }); } catch (e) { throw e; } },
            updateMainData: async (id, name, description, updatedById) => { try { return await AxiosManager.post('/VendorCategory/UpdateVendorCategory', { id, name, description, updatedById }); } catch (e) { throw e; } },
            deleteMainData: async (id, deletedById) => { try { return await AxiosManager.post('/VendorCategory/DeleteVendorCategory', { id, deletedById }); } catch (e) { throw e; } },
        };
        ...
        const handler = {
            openAdd: () => { state.deleteMode = false; state.mainTitle = 'Add Vendor Category'; resetFormState(); showModal(); },
            openEdit: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = false; state.mainTitle = 'Edit Vendor Category'; mainGrid._populateState(row); showModal(); },
            openDelete: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = true; state.mainTitle = 'Delete Vendor Category?'; mainGrid._populateState(row); showModal(); },
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) { state.mainTitle = 'Edit Vendor Category'; state.id = response?.data?.content?.data.id ?? ''; state.name = response?.data?.content?.data.name ?? ''; state.description = response?.data?.content?.data.description ?? ''; }
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['VendorCategories']); await SecurityManager.validateToken();
        ...
        const mainGrid = {
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Vendor Categories', exportOptions: { columns: ':visible' } }],
