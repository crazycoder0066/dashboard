const App = {
    setup() {
        ...
        const services = {
            getMainData: async () => { try { return await AxiosManager.get('/Todo/GetTodoList', {}); } catch (e) { throw e; } },
            createMainData: async (name, description, createdById) => { try { return await AxiosManager.post('/Todo/CreateTodo', { name, description, createdById }); } catch (e) { throw e; } },
            updateMainData: async (id, name, description, updatedById) => { try { return await AxiosManager.post('/Todo/UpdateTodo', { id, name, description, updatedById }); } catch (e) { throw e; } },
            deleteMainData: async (id, deletedById) => { try { return await AxiosManager.post('/Todo/DeleteTodo', { id, deletedById }); } catch (e) { throw e; } },
        };
        ...
        const resetFormState = () => { state.id = ''; state.name = ''; state.description = ''; state.errors = { name: '', description: '' }; };

        const handler = {
            openAdd: () => { state.deleteMode = false; state.mainTitle = 'Add Todo'; resetFormState(); showModal(); },
            openEdit: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = false; state.mainTitle = 'Edit Todo'; mainGrid._populateState(row); showModal(); },
            openDelete: () => { const row = mainGrid.obj?.rows({ selected: true }).data()[0]; if (!row) return; state.deleteMode = true; state.mainTitle = 'Delete Todo?'; mainGrid._populateState(row); showModal(); },
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['Todos']); await SecurityManager.validateToken();
        ...
        const mainGrid = {
            ...
            create: (dataSource) => {
                mainGrid.obj = new DataTable('#mainGrid', {
                    ...
                    buttons: [{ extend: 'excelHtml5', title: 'Todos', exportOptions: { columns: ':visible' } }],
